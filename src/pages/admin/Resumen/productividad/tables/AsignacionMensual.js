...
export const AsignacionMensual = () => {
  ...
  const { columns, tableData } = useMemo(() => {
    ...
    const rawData = data?.total_mensual?.data || [];
    const promediosData = data?.promedio_mensual?.data?.[0] || {};
    const promediosTipificacion = promediosData.promedio_por_tipificacion || [];
    ...
    promediosTipificacion.forEach((item) => {
      const key = `${item._id.year}_${item._id.tipificacion}`; // Ej: "2026_PQRS"
      if (!rowsMap[key]) {
        // Por si la API devuelve un promedio de un año sin meses asociados en el filtro
        rowsMap[key] = { anio: item._id.year, concepto: item._id.tipificacion, esTotal: false };
      }
